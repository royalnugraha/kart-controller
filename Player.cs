...
public class Player : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (RACE_MANAGER.RACE_STARTED && !item_manager.isBullet && !RACE_MANAGER.RACE_COMPLETED)
        {
            ...
            //dust particles
            if (REALCURRENTSPEED > 30 && !drift_left && !drift_right && !GLIDER_FLY && !JUMP_PANEL && !Input.GetKey(KeyCode.DownArrow) && grounded && !item_manager.isBullet && Is_Dirt_Track)
            {
    ...
    void Move()
    {
        ...
        //reverse
        if (Input.GetKey(KeyCode.DownArrow) && !GLIDER_FLY)
        {
        ...
        if((Input.GetKey(KeyCode.UpArrow) && REALCURRENTSPEED < 0) || (currentspeed > 40 && REALCURRENTSPEED <= 5 && Input.GetKey(KeyCode.UpArrow)))//skid effect
        {
            reverseSkid = true;    
        }

        if (reverseSkid && REALCURRENTSPEED < 20 && Input.GetKey(KeyCode.UpArrow) && !IN_WATER)
        {
            AccelBeforeStartDust.GetChild(0).GetComponent<ParticleSystem>().Play();
            AccelBeforeStartDust.GetChild(1).GetComponent<ParticleSystem>().Play();
        ...
        if(Input.GetKey(KeyCode.DownArrow) && !Input.GetKey(KeyCode.UpArrow) && REALCURRENTSPEED < 0 && !SpecialFace)
        {
            reversing = true;
            Driver.SetBool("Reverse", true);
            reversingTime += Time.deltaTime;
        }
        else if (Input.GetKeyUp(KeyCode.DownArrow) || Input.GetKeyDown(KeyCode.UpArrow)){
            current_face_material = faces[0];
    ...
    void Steer()
    {
        ...
        if (Input.GetAxis("Horizontal") != 0)
        {
            ...
            if (REALCURRENTSPEED < -5 && !Input.GetKey(KeyCode.UpArrow)) //reverse
                speed_rotate_rate = -0.5f;
    ...
    void movingCarParts()
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            if (Input.GetKey(KeyCode.UpArrow) && REALCURRENTSPEED < 0)
            {
    ...
    void tireDirtEffect()
    {
        if (Is_Dirt_Track)
        {
            if (Input.GetKey(KeyCode.UpArrow) && !item_manager.isBullet && !item_manager.StarPowerUp)
            {
                for(int i = 0; i < tireRenderers.Length; i++)
                {
